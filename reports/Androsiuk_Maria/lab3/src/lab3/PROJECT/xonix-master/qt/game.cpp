...
QBrush bgBrush(QColor(1,255,255));
QString valuetext = "Arial";
QMap<int,int> keyMap;
const int period=17;
...
void Game::keyPressEvent( QKeyEvent *keyEvent)
{
    ...
    if(k == Qt::Key_1) {
        typedef QString (*Font_set_first)();
        // установка имени первого заголовка
        QLibrary *helper_set_f_n = new QLibrary("Font");
        Font_set_first set_window_name_first = (Font_set_first)helper_set_f_n->resolve("set_firstfont");
        QString set_name_first = set_window_name_first();
        delete helper_set_f_n;
        valuetext = set_name_first;
        QMessageBox *mess = new QMessageBox;
        mess->setText("Смена шрифта на " + valuetext);
        mess->exec();
    }
    if(k == Qt::Key_2) {
        typedef QString (*Font_set_second)();
        // установка имени второго заголовка
        QLibrary *helper_set_s_n = new QLibrary("Font");
        Font_set_second set_window_name_second = (Font_set_second)helper_set_s_n->resolve("set_secondfont");
        QString set_name_second = set_window_name_second();
        delete helper_set_s_n;
        valuetext = set_name_second;
        QMessageBox *mess = new QMessageBox;
        mess->setText("Смена шрифта на " + valuetext);
        mess->exec();
    }
    if(k == Qt::Key_3) {
        typedef QString (*Font_set_last)();
        QLibrary *helper_set_r_n = new QLibrary("Font");
        Font_set_last set_window_name_last = (Font_set_last)helper_set_r_n->resolve("set_lastfont");
        QString set_name_last = set_window_name_last();
        delete helper_set_r_n;
        valuetext = set_name_last;
        QMessageBox *mess = new QMessageBox;
        mess->setText("Смена шрифта на " + valuetext);
        mess->exec();
    }
