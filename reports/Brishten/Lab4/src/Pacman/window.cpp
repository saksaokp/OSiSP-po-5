    ...
    ui(new Ui::window)
{
    ...
    //connect(levelMenu, SIGNAL(triggered(QAction*)), this, SLOT(onRecentOpenFiles(QAction*)));
    ...
    if (socket->waitForConnected(500))
    {
        ...
        if(testing != "")
        {
            ...
            for (int i = 0; i < list.size() - 1; i++) {
                QFile target;
                target.setFileName("D:/5 semester/OSISP/Lab4/Pacman-build/debug/updates/level" + QString::number(fileNum) + ".dll");
            ...
            QMessageBox::information(this, "Информация", "Обновление успешно загружено");
            loadLevel();
        }

        else
        {
            QMessageBox::information(this, "Информация", "Установлены последние обновления");
        }
    }
}
