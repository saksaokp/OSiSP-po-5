...
Arkanoid::~Arkanoid()
{
    ...
    for (int i=0; i< Helper_class::BRICKS_COUNT; i++) {
        delete bricks[i];
    }
}
...
{
    dialogStart* dialog = new dialogStart(this);
    dialog->setModal(true);
    dialog->exec();

    if (dialog->operation.compare("startGame") == 0) {
        if (!gameStarted) {
            Helper_class::chooseLevel(bricks, dialog->lvl);                                          //*****************
    ...
    else if (dialog->operation.compare("checkUpdates") == 0) {
        this->close();
    }
}
...
void Arkanoid::checkCollision()
{
    ...
    if (count == Helper_class::BRICKS_COUNT) {
        winGame();
    }
